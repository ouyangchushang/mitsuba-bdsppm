 ...
 * }
 */
class BDPMIntegrator : public MonteCarloIntegrator {
public:
    ...
    BDPMIntegrator(const Properties &props) : MonteCarloIntegrator(props) {
    ...
    // The following part is added by us.
    // To estimate the irradiance at a given surface point
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        rRec.rayIntersect(ray);//finding the intersection of rays
        ray.mint = Epsilon;

        Spectrum throughput(1.0f);
        Float eta = 1.0f;   
        int fakedepth = 0;  //to record the number of reflections
        //we need to record the depth of our ray and also make sure that reflections are not counted.

        while (rRec.depth <= m_maxDepthRay || m_maxDepthRay < 0) {//to make sure that the number of ray segments does not exceed our stipulated maximum value.
            /*first, we consider different scenarios that the ray encounters*/
            if (!its.isValid() && rRec.depth==1) {//if the ray does not encounter any object
                if ((rRec.type & RadianceQueryRecord::EEmittedRadiance)
                    && (!m_hideEmitters || scattered))
                    Li += throughput * scene->evalEnvironment(ray);//then the only contribution to the radiance is the environment light
                break;
            }
            ...
            if (rRec.depth==1){
                if(its.isEmitter()){ 
                    if((rRec.type & RadianceQueryRecord::EEmittedRadiance) && (!m_hideEmitters || scattered)){
                        Li += throughput * its.Le(-ray.d);//if the ray intersects with a light source
                    }
                }
                if(its.hasSubsurface()){ 
                    if(rRec.type & RadianceQueryRecord::ESubsurfaceRadiance){
                        Li += throughput * its.LoSub(scene, rRec.sampler, -ray.d, rRec.depth);// if the ray encounters a surface
                    }
                }
            }
            ...
            /*below, we may need to record some of the variants stated in the paper*/ 
            std::vector<Float> rayPdf, rayInvPdf;  //the first two are the possibilities 
            std::vector<Spectrum> vecInvEval, vecThroughput;  //these two are the sampling traits
            std::vector<Vector> vecWi;     //the last one is the incoming direction

            
             
            Spectrum flux;
            
            Float M = (Float) m_photonMap->estimateRadianceRawNew(its, m_initialRadius, flux, (m_maxDepthRay == -1 ? INT_MAX : m_maxDepth), rRec.depth, m_rrDepth, rayPdf, rayInvPdf, vecInvEval, throughput, vecWi);
            Li += throughput * flux/ ((Float) m_totalEmitted * m_initialRadius * m_initialRadius * M_PI);  
            /*considering all the possibilities, evals... then calculate the contribution to the current pixal*/
            
             

            /* now we can continue to sample the next point */
            Float bsdfPdf;
            ...
            /*although we do need to exclude 2 scenarios*/
            //1. reflection
            if(bRec.sampledType == BSDF::EDeltaReflection)
            {
                fakedepth++;
                if(fakedepth<10)//making sure that the number of reflection does not exceed 10
                {
                    ...
                    throughput = throughput * bsdfWeight;
                    eta = eta * (bRec.eta);
            ...
            //2. if the loop depth exceeds the maximum
            ...
            //here we need to calculate new InvEvals etc. to push into the corresponding stacks 
            Spectrum tmpInvEval;
            ...
            //calculating new InvPdf and InvEval based on BSDF
            if(tmpbRec.sampledType == BSDF:: EDeltaReflection){
                tmpInvPdf = bsdf->pdf(tmpbRec, EDiscrete);
                tmpInvEval = bsdf->eval(tmpbRec, EDiscrete);
            }
            else{
                tmpInvPdf = bsdf->pdf(tmpbRec, ESolidAngle);
                tmpInvEval = bsdf->eval(tmpbRec, ESolidAngle);
            } 
            ...
            throughput = throughput * bsdfWeight;
            eta = eta * (bRec.eta);//updating the depth
...
MTS_IMPLEMENT_CLASS_S(BDPMIntegrator, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(BDPMIntegrator, "Bidirectional progressive photon mapper");
MTS_NAMESPACE_END